      ...
      >
        ...
        <Grid container spacing={0} sx={{ mt: 2 }}>
          ...
          >
            ...
            <Select
              onChange={(e) =>
                handleInputChange({
                  target: { name: "ActivityID", value: e.target.value },
                })
              }
              name="ActivityID"
              fullWidth
        ...
        <Grid container spacing={0} sx={{ mt: 3 }}>
          <Grid xs={12} sm={12} md={12} lg={6} xl={6} sx={{ pl: 2, pr: 1 }}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                ...
                value={
                  formData.StartDate
                    ? dayjs(moment(formData?.StartDate).format("YYYY-MM-DD"))
                    : null
                }
                onChange={(date) =>
                  handleInputChange({
          ...
          <Grid
            ...
            lg={6}
            xl={6}
            sx={{ pl: 2, pr: 1, pt: { xs: 2, sm: 2, md: 2, lg: 0, xl: 0 } }}
          >
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                label="End Date"
                value={
                  formData.EndDate
                    ? dayjs(moment(formData?.EndDate).format("YYYY-MM-DD"))
                    : null
                }
                onChange={(date) =>
                  handleInputChange({
        ...
        <Grid container spacing={0} sx={{ mt: 2 }}>
          ...
          >
            ...
            >
              {data?.baseData?.statusData?.map((data: any) => (
                <MenuItem value={data?.StatusID}>{data?.StatusName}</MenuItem>
              ))}
            </Select>
          </Grid>
          <Grid
          ...
          >
            ...
            <Select
              onChange={(e) =>
                handleInputChange({
                  target: { name: "PriorityID", value: e.target.value },
                })
              }
              name="PriorityID"
              required={true}
            ...
            >
              {data?.baseData?.priorityData?.map((data: any) => (
                <MenuItem value={data?.PriorityID}>
                  {data?.PriorityName}
                </MenuItem>
              ))}
            </Select>
          ...
          >
            ...
            <Select
              onChange={(e) =>
                handleInputChange({
                  target: { name: "SeverityID", value: e.target.value },
                })
              }
              name="SeverityID"
              required={true}
            ...
            >
              {data?.baseData?.severityData?.map((data: any) => (
                <MenuItem value={data?.SeverityID}>
                  {data?.SeverityName}
                </MenuItem>
              ))}
            </Select>
        ...
        <Grid container spacing={0} sx={{ mt: 2 }}>
          ...
          <Grid
            ...
            lg={6}
            xl={6}
            sx={{ pr: 1, pt: { xs: 2, sm: 2, md: 2, lg: 0, xl: 0 }, pl: 2 }}
          >
            <InputLabel id="demo-simple-select-standard-label">
              Assignee
            </InputLabel>
            ...
            >
              {data?.baseData?.assigneeData?.map((data: any) => (
                <MenuItem value={data?.UserID}>{data?.UserName}</MenuItem>
              ))}
            </Select>
          </Grid>
        </Grid>
        <Grid container spacing={1} sx={{ mt: 2, pb: 2 }}>
          <Grid
            ...
            lg={6}
            xl={6}
            sx={{ width: "100%", textAlign: "right", pt: 2 }}
          >
            <Button
              variant="contained"
              type="submit"
          ...
          <Grid
            ...
            lg={6}
            xl={6}
            sx={{ width: "100%", textAlign: "left", pl: 2, pt: 2 }}
          >
            <Button
              variant="contained"
              sx={{
