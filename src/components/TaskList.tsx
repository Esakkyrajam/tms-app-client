    ...
    <>
      ...
      >
        {data?.TaskListData?.length > 0 ? (
          ...
          >
            <Table
              data-testid={configs?.dataTestID}
              sx={{ minWidth: 650 }}
            ...
            >
              ...
              <TableBody>
                {data?.TaskListData?.map((data: any, index: any) => (
                  ...
                  >
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "Title"
                    ) && <TableCell>{data?.Title}</TableCell>}
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "TaskCode"
                    ) && <TableCell>{data?.TaskCode}</TableCell>}
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "StatusID"
                    ) && (
                      <TableCell>
                    ...
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "AssignedUserID"
                    ) && (
                      <TableCell>
                        {assigneeData && data.AssignedUserID
                          ? assigneeData.find(
                              (user: any) => user.UserID === data.AssignedUserID
                            )?.UserName
                          : "-"}
                      </TableCell>
                    )}
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "StartDate"
                    ) && (
                      <TableCell>
                        {helper.converttoDateFormat(
                          data?.StartDate,
                          "MM/DD/YYYY"
                        )}
                      </TableCell>
                    ...
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "EndDate"
                    ) && (
                      <TableCell>
                        {helper.converttoDateFormat(
                          data?.EndDate,
                    ...
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "CreatedBy"
                    ) && <TableCell>{data?.CreatedBy}</TableCell>}
                    {TaskListHeaders?.find(
                      (data: any) => data?.ColumnName === "CreatedDate"
                    ) && (
                      <TableCell>
                        {helper.converttoDateFormat(
                          data?.CreatedDate,
                    ...
                    <TableCell data-testid={data?.TaskCode + "edit"}>
                      <IconButton
                        onClick={() => callbacks.handleTaskEdit(data)}
                      >
                        <span
                    ...
                    <TableCell
                      data-testid={data?.TaskCode + "delete"}
                      sx={{
                        display: "flex",
                    ...
                    >
                      <IconButton
                        onClick={() => callbacks.handleTaskDelete(data)}
                      >
                        <span
                          className="material-icons"
                          style={{
